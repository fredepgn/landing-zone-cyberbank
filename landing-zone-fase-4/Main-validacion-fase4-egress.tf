...
# Ruta adicional de salida a Internet en la RT de Digital Channels
resource "aws_route" "digital_channels_to_internet" {
  route_table_id         = aws_route_table.digital_channels_private.id
  destination_cidr_block = "0.0.0.0/0"
  transit_gateway_id     = aws_ec2_transit_gateway.tgw.id
}

# Ruta adicional en la RT temporal del TGW para Egress
resource "aws_ec2_transit_gateway_route" "digital_channels_to_egress" {
  transit_gateway_route_table_id = aws_ec2_transit_gateway_route_table.temp_validation.id
  destination_cidr_block         = "0.0.0.0/0"
  transit_gateway_attachment_id  = aws_ec2_transit_gateway_vpc_attachment.egress.id
}

# Asociación del attachment de Egress a la RT temporal
resource "aws_ec2_transit_gateway_route_table_association" "egress_temp" {
  transit_gateway_attachment_id  = aws_ec2_transit_gateway_vpc_attachment.egress.id
  transit_gateway_route_table_id = aws_ec2_transit_gateway_route_table.temp_validation.id
}
